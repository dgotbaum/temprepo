...
public:
	...
	UPROPERTY()
	TMap<FString, FString> ContentMultipartParameters;
	UPROPERTY()
	FString GeneratedContentMultipartBoundary;
	UPROPERTY()
	...
	UPROPERTY()
	UObject* ExecutionEventObject = NULL;
	UPROPERTY()
	...
	*/
	...
	/**
	* Sets the content as POST parameters (application/x-www-form-urlencoded) (this will automatically add the required headers to send a POST request).
	* 
	* Call this blueprint multiple times to have multiple parameters.
	*/
	...
	/**
	* Sets the content as multipart form parameters (multipart/form-data) (this will automatically add the required headers to send a multipart request).
	*
	* The given string will automatically be converted to bytes.
	*
	* Call this blueprint multiple times to have multiple parameters.
	*/
	UFUNCTION(BlueprintCallable, Category = "Low Entry|Http Request|Request", Meta = (DisplayName = "Set Content As Multiparts (String)", Keywords = "post put form data parameters", AdvancedDisplay = "2"))
		void SetContentAsMultipartParameters(const FString& Name, const FString& Content, const FString& OptionalContentType, const FString& OptionalFileName);
	...
	* Call this blueprint multiple times to have multiple parameters.
	*/
	UFUNCTION(BlueprintCallable, Category = "Low Entry|Http Request|Request", Meta = (DisplayName = "Set Content As Multiparts (Bytes)", Keywords = "post put form data parameters", AdvancedDisplay = "2"))
		void SetContentAsMultipartParametersBytes(const FString& Name, const TArray<uint8>& Content, const FString& OptionalContentType, const FString& OptionalFileName);
	...
	*/
	...
	/**
	* Returns the set content parameters.
	*/
	UFUNCTION(BlueprintPure, Category = "Low Entry|Http Request|Request", Meta = (DisplayName = "Get Content As Parameters"))
		FString GetContentAsStringParameters();

	/**
	* Returns the set content multiparts.
	*/
	UFUNCTION(BlueprintCallable, Category = "Low Entry|Http Request|Request", Meta = (DisplayName = "Get Content As Multiparts"))
		void GetContentAsMultipartParameters(FString& Boundary, TArray<uint8>& Content);


	/**
	* Returns the URL (including the url parameters) and the type.
	*/
	UFUNCTION(BlueprintPure, Category = "Low Entry|Http Request|Request", Meta = (DisplayName = "Get Url"))
		void GetUrl(FString& Type, FString& Url);

	/**
	* Returns the headers.
	*/
	UFUNCTION(BlueprintPure, Category = "Low Entry|Http Request|Request", Meta = (DisplayName = "Get Headers"))
		TMap<FString, FString> GetHeaders();

	/**
	* Returns the content (as bytes).
	*/
	UFUNCTION(BlueprintPure, Category = "Low Entry|Http Request|Request", Meta = (DisplayName = "Get Content"))
