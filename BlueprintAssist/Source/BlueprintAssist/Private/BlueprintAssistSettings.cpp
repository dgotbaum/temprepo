	...
	: Super(ObjectInitializer)
{
	...
	BlueprintFormatterSettings.AutoFormatting = BAAutoFormat_FormatAllConnected;
	
	BehaviorTreeFormatterSettings.Padding = FVector2D(100, 100);
	BehaviorTreeFormatterSettings.AutoFormatting = BAAutoFormat_FormatAllConnected;
	...
	NiagaraGraphFormatterSettings.AutoFormatting = BAAutoFormat_FormatAllConnected;
	
	AnimationGraphFormatterSettings.Padding = FVector2D(80, 150);
	AnimationGraphFormatterSettings.AutoFormatting = BAAutoFormat_FormatAllConnected;

	SoundCueGraphFormatterSettings.Padding = FVector2D(80, 150);
	SoundCueGraphFormatterSettings.AutoFormatting = BAAutoFormat_FormatAllConnected;
		...
		if (InputProcessor->TabHandler.IsValid())
		{
			...
			if (GraphHandler.IsValid())
			{
				...
				else if (PropertyName == GET_MEMBER_NAME_CHECKED(UBASettings, bCacheSizeWhenRequired))
				{
					...
					|| PropertyName == GET_MEMBER_NAME_CHECKED(UBASettings, bUseCommentBubbleWidth)
