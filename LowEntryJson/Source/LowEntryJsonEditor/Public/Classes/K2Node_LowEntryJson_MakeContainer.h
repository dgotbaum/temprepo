...
#include "K2Node_LowEntryJson_MakeContainer.generated.h"

class FBlueprintActionDatabaseRegistrar;
class UEdGraphPin;

class FKCHandler_LowEntryJson_MakeContainer : public FNodeHandlingFunctor
{
public:
	FKCHandler_LowEntryJson_MakeContainer(FKismetCompilerContext& InCompilerContext)
...
class UK2Node_LowEntryJson_MakeContainer : public UK2Node, public IK2Node_AddPinInterface
{
...
public:
	LOWENTRYJSONEDITOR_API virtual void RemoveInputPin(UEdGraphPin* Pin);

	LOWENTRYJSONEDITOR_API UEdGraphPin* GetOutputPin() const;

	/** returns a reference to the output array pin of this node, which is responsible for defining the type */
	virtual FName GetOutputPinName() const PURE_VIRTUAL(UK2Node_LowEntryJson_MakeContainer::GetOutputPinName, return NAME_None;);
...
public:
	...
	virtual class FNodeHandlingFunctor* CreateNodeHandler(class FKismetCompilerContext& CompilerContext) const override PURE_VIRTUAL(UK2Node_LowEntryJson_MakeContainer::CreateNodeHandler, return nullptr;);
	...
	LOWENTRYJSONEDITOR_API virtual void AddInputPin() override;
