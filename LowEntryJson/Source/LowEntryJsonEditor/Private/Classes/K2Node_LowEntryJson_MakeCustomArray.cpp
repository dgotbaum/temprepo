...
#include "K2Node_LowEntryJson_MakeCustomArray.h"
...
/////////////////////////////////////////////////////
// FKCHandler_LowEntryJson_MakeCustomArray

class FKCHandler_LowEntryJson_MakeCustomArray : public FNodeHandlingFunctor
{
public:
	FKCHandler_LowEntryJson_MakeCustomArray(FKismetCompilerContext& InCompilerContext) : FNodeHandlingFunctor(InCompilerContext)
	{
	}

	virtual void RegisterNets(FKismetFunctionContext& Context, UEdGraphNode* Node) override
	{
		UK2Node_LowEntryJson_MakeCustomArray* ArrayNode = CastChecked<UK2Node_LowEntryJson_MakeCustomArray>(Node);
...
/////////////////////////////////////////////////////
// UK2Node_LowEntryJson_MakeCustomArray

UK2Node_LowEntryJson_MakeCustomArray::UK2Node_LowEntryJson_MakeCustomArray(const FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	ContainerType = EPinContainerType::Array;
}

FNodeHandlingFunctor* UK2Node_LowEntryJson_MakeCustomArray::CreateNodeHandler(FKismetCompilerContext& CompilerContext) const
{
	return new FKCHandler_LowEntryJson_MakeCustomArray(CompilerContext);
}

FText UK2Node_LowEntryJson_MakeCustomArray::GetNodeTitle(ENodeTitleType::Type TitleType) const
{
	return FText::FromString(LowEntry_Name);
}

FName UK2Node_LowEntryJson_MakeCustomArray::GetOutputPinName() const
{
	return TEXT("TempArray");
}

UEdGraphPin* UK2Node_LowEntryJson_MakeCustomArray::GetArrayOutputPin() const
{
	return FindPin(GetOutputPinName());
}

UEdGraphPin* UK2Node_LowEntryJson_MakeCustomArray::GetObjectOutputPin() const
{
	return FindPin(LowEntry_ResultPinName);
}

FText UK2Node_LowEntryJson_MakeCustomArray::GetTooltipText() const
{
	return FText::FromString(LowEntry_Tooltip);
}

FSlateIcon UK2Node_LowEntryJson_MakeCustomArray::GetIconAndTint(FLinearColor& OutColor) const
{
	static FSlateIcon Icon("EditorStyle", "GraphEditor.MakeArray_16x");
	return Icon;
}

bool UK2Node_LowEntryJson_MakeCustomArray::IsNodePure() const
{
	return LowEntry_IsNodePure;
}

void UK2Node_LowEntryJson_MakeCustomArray::AllocateDefaultPins()
{
...
void UK2Node_LowEntryJson_MakeCustomArray::PostReconstructNode()
{
	Super::Super::PostReconstructNode();
}

void UK2Node_LowEntryJson_MakeCustomArray::NotifyPinConnectionListChanged(UEdGraphPin* Pin)
{
	Super::Super::NotifyPinConnectionListChanged(Pin);
}

void UK2Node_LowEntryJson_MakeCustomArray::GetNodeContextMenuActions(UToolMenu* Menu, UGraphNodeContextMenuContext* Context) const
{
	...
	if(!Context->bIsDebugging)
	{
		...
		if(Context->Pin != NULL)
		{
			if((Context->Pin->Direction == EGPD_Input) && (Context->Pin->ParentPin == nullptr) && (Context->Pin->PinType.PinCategory != UEdGraphSchema_K2::PC_Exec))
			{
				Section.AddMenuEntry(
					...
					FSlateIcon(),
					FUIAction(
						FExecuteAction::CreateUObject(const_cast<UK2Node_LowEntryJson_MakeCustomArray*>(this), &UK2Node_LowEntryJson_MakeCustomArray::RemoveInputPin, const_cast<UEdGraphPin*>(Context->Pin))
					)
				);
			}
		}
		else
		{
			Section.AddMenuEntry(
				...
				FSlateIcon(),
				FUIAction(
					FExecuteAction::CreateUObject(const_cast<UK2Node_LowEntryJson_MakeCustomArray*>(this), &UK2Node_LowEntryJson_MakeCustomArray::InteractiveAddInputPin)
				)
			);
		}
	}
...
void UK2Node_LowEntryJson_MakeCustomArray::ReallocatePinsDuringReconstruction(TArray<UEdGraphPin*>& OldPins)
{
	Super::Super::ReallocatePinsDuringReconstruction(OldPins);
}

void UK2Node_LowEntryJson_MakeCustomArray::ValidateNodeDuringCompilation(class FCompilerResultsLog& MessageLog) const
{
	Super::Super::ValidateNodeDuringCompilation(MessageLog);
}

FText UK2Node_LowEntryJson_MakeCustomArray::GetMenuCategory() const
{
	return FText::FromString(LowEntry_Category);
}

bool UK2Node_LowEntryJson_MakeCustomArray::IsConnectionDisallowed(const UEdGraphPin* MyPin, const UEdGraphPin* OtherPin, FString& OutReason) const
{
