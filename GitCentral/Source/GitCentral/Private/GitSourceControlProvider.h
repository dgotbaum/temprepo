...
public:
	...
	//Note: this has only minor cosmetic impact on diff UI strings (verified @ UE4.17)
	virtual bool UsesChangelists() const override;

	//Note: this is currently only used to display or not the "keep files checked out" option on the submit dialog (verified @ UE4.17)
	...
	/** Reset state cache, useful after repository settings have been altered. */
	void ClearCache();
	...
	inline const FString& GetBranch() const
	{
		return BranchName;
	}

	/** Current default remote */
	inline const FString& GetRemote() const
	{
		return RemoteName;
	}
	...
	const TMap<FString, TSharedRef<class FGitSourceControlState, ESPMode::ThreadSafe>>& GetAllStatesInternal() { return StateCache; }
...
private:
	...
	/** True if we managed to establish a connection to the remote */
	bool bConnected;

	/** Will broadcast the system for update next tick */
	bool bForceBroadcastUpdateNextTick;
