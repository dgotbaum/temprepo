...
bool FGitConnectWorker::UpdateStates() const
{
	...
	GitSourceControl.GetStatusFile().Load(Provider.GetPathToRepositoryRoot());
...
bool FGitCheckOutWorker::Execute(class FGitSourceControlCommand& InCommand)
{
	...
	for(const auto& State : LocalStates)
	{
		...
		if(FileState.State == EWorkingCopyState::Unknown || FileState.State == EWorkingCopyState::Unchanged)
		{
			...
			StatusFile.SetState(File, NewFileState, InCommand.PathToRepositoryRoot, true);
		}
	}
...
bool FGitCheckInWorker::Execute(FGitSourceControlCommand& InCommand)
{
	...
	for(const auto& State : LocalStates)
	{
		if(!State->CanCheckIn())
		{
			InCommand.Files.RemoveSingle(State->GetFilename());
		}
	}
	...
	{
		InCommand.bCommandSuccessful = GitSourceControlUtils::RunUnlockFiles(InCommand.PathToGitBinary, InCommand.PathToRepositoryRoot, InCommand.Remote, InCommand.Files, InCommand.ErrorMessages);
	}
...
bool FGitSyncWorker::Execute(class FGitSourceControlCommand& InCommand)
{
	...
	else //command is called for directories and/or individual files, sync them
	{
		...
		for(const auto& State : LocalStates)
		{
			if(State->IsCurrent())
			{
				FilesToSync.RemoveSingle(State->GetFilename());
			}
		}
...
bool FGitSyncWorker::GetLatest(class FGitSourceControlCommand& InCommand)
{
	...
	if(MergeBase.IsEmpty() || MergeBase == RemoteSha) //Nothing to sync
		return false;
			...
			for(auto It : StatusResult)
			{
				if(It.Value.IsConflicted())
				{
					...
					if(LocalStates[0]->IsConflicted()) //Only keep conflict state if we hadn't resolve the conflict already
					{
						...
						StatusFile.SetState(File, FileState, InCommand.PathToRepositoryRoot, true);
					}
				}
			}
	...
	for(auto It : SavedStatuses)
	{
		...
		StatusFile.SetState(It.Key, FileState, InCommand.PathToRepositoryRoot, true);
	}
