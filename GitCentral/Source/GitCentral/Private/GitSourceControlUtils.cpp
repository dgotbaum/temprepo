...
FString FindGitBinaryPath()
{
	...
	bool bFound = !GitBinaryPath.IsEmpty() && FPaths::FileExists(GitBinaryPath) && CheckGitAvailability(GitBinaryPath);
...
M       modified_file.txt
*/
static void ParseNameStatusResults(const FString& InPathToGitBinary, const FString& InRepositoryRoot, const TArray<FString>& InResults, TMap<FString, FGitSourceControlState>& OutStates)
{
	// Iterate on results first
	for(const auto& Line : InResults)
	{
...
bool RunUpdateStatus(FGitSourceControlCommand& InCommand, const TArray<FString>& InFiles, TArray<FString>& OutErrorMessages, TArray<FGitSourceControlState>& OutStates)
{
	...
	// Run regular git status to update local status
	{
		...
		TArray<FString> ErrorMessages;
		bool bResult = RunCommand(TEXT("status --porcelain --ignored"), InPathToGitBinary, InRepositoryRoot, TArray<FString>(), FilesParam, Results, ErrorMessages);
	...
	for(auto& It : States)
	{
		...
		{
			if(It.Value.WorkingCopyState != EWorkingCopyState::Unknown) //Fix for newly created assets, if something didn't have a status at all and doesn't exist on disk it's probably not deleted
