...
void FGitSourceControlMenuExtensions::AddSourceControlMenuExtension(FMenuBuilder& MenuBuilder) const
{
...
void FGitSourceControlMenuExtensions::AddAssetContextMenuExtension(FMenuBuilder& MenuBuilder) const
{
...
void FGitSourceControlMenuExtensions::OnGetLatest()
{
	...
	//get latest
	ISourceControlProvider& SCCProvider = ISourceControlModule::Get().GetProvider();
	auto Result = SCCProvider.Execute(ISourceControlOperation::Create<FSync>(), Provider.GetPathToRepositoryRoot());
	if (Result == ECommandResult::Succeeded)
	{
		//Skipping editor restart for now, will restore it if testing proves it is a better choice.
		/*FSuppressableWarningDialog::FSetupInfo Info(
			LOCTEXT("GetLatest_RestartEditor", "Restart the editor? \nRestarting ensures proper reload of the assets after getting latest."),
			LOCTEXT("GetLatest_RestartEditor_Title", "Restart Editor"),
			TEXT("GitCentralRestartEditorWarning"));
		...
		auto ConfirmRestart = OSUpdateWarning.ShowModal();
		...
		{
			//Note: If we have unsaved packages this will prompt for save again
	...
	{
		FMessageLog EditorErrors("EditorErrors");
		EditorErrors.Warning(LOCTEXT("GetLatestFailed", "Failed to Get Latest, you may be up to date, otherwise check the logs for errors !"));
		EditorErrors.Notify();
		ReloadPackages(PackagesToReload);
	}
}
